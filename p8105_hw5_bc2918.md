...
effect the power. As the effect size deviating from 0, the proportion of
rejecting null hypothesis gets higher and higher, which makes sense
because the true mean now indeed is not 0 and the null should be
rejected more and more likely as the difference gets larger.

Make a plot showing the average estimate of μ̂ (of total and of rejected
samples) on the y axis and the true value of μ on the x axis.
