...
In this plot, the average estimate of the sample means all nicely
approximate the true means. However, it could be observed that the
sample average of μ̂ across tests for which the null is rejected
`deviates` more from the true value of μ when the true value of μ is
actually close to the 0 (which is the value we set in the null
hypothesis), and `gets closer` to the true mean when the true mean
...
means of these samples will just approximate the true mean.
