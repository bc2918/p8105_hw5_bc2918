...
proportion of rejected null hypothesis, gets higher and higher, which
makes sense because the true mean now indeed is not 0 and the null
should be rejected more and more likely as the difference gets larger.
When the difference is large enough, the power approaches 1.0.
...
It could be observed that the sample average of μ̂ across tests for
which the null is rejected `deviates more` from the true value of μ when
the true value of μ is actually close to the 0 (which is the value we
set in the null hypothesis), and `gets closer` to the true mean when the
true mean gets larger and more different from the nul. It makes sense
because when the true mean is closer to the null, the cases get rejected
are more of extreme samples that deviates from the true mean, while when
the true mean is drastically different from the null, most cases will be
rejected, the means of which will just approximate the true mean.
