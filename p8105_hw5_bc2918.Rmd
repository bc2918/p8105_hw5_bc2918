...
In this plot, the average estimate of the sample means all nicely approximate the true means. However, it could be observed that the sample average of μ̂  across tests for which the null is rejected `deviates` more from the true value of μ when the true value of μ is actually close to the 0 (which is the value we set in the null hypothesis), and `gets closer` to the true mean when the true mean differs more from the null. It makes sense because when the true mean is closer to the null, the cases get rejected are more of extreme samples that deviates even more from the true mean, while when the true mean is drastically different from the null, most cases will be rejected and the means of these samples will just approximate the true mean. 
